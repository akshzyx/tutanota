...
use tutasdk::entities::generated::tutanota::{ImportAttachment, ImportMailData, ImportMailPostIn, NewImportAttachment};
...
impl Importer {
		...
		Iter: Iterator<Item = ImportableMail>,
	{
        ...
        let mail_group_key = self
            ...
            .await
            .map_err(|_e| ())?;
        let owner_enc_mail_session_key =
            mail_group_key.encrypt_key(&new_mail_aes_256_key, Iv::generate(&self.randomizer_facade));

        const MAX_REQUEST_SIZE: usize = 1024 * 1024 * 5;
		let import_mail_data_and_attachments = importable_mails.map(<(ImportMailData, Vec<ImportableMailAttachment>)>::from);
        ...
        for imports in import_chunks {
            ...
            for (import_mail_data, importable_mail_attachments) in imports.into_iter() {
                ...
                for importable_mail_attachment in importable_mail_attachments {
                    let importable_mail_attachment = importable_mail_attachment as ImportableMailAttachment;
                    ...
                    let reference_tokens = self.logged_in_sdk
                        .blob_facade()
                        .encrypt_and_upload(
                    ...
                    let enc_file_name = new_file_aes_256_key.encrypt_data(importable_mail_attachment.filename.as_ref(), Iv::generate(&self.randomizer_facade)).unwrap();
                    let enc_mime_type = new_file_aes_256_key.encrypt_data(importable_mail_attachment.content_type.as_ref(), Iv::generate(&self.randomizer_facade)).unwrap();
                    let enc_cid: Option<Vec<u8>> = match importable_mail_attachment.content_id {
                        Some(cid) => Some(new_file_aes_256_key.encrypt_data(cid.as_bytes(), Iv::generate(&self.randomizer_facade)).unwrap()),
                        None => None,
                    };
